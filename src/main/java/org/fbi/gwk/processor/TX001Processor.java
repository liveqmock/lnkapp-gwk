...
import org.fbi.gwk.domain.cbs.T1000Response.CbsToa1000Item;
...
import org.fbi.gwk.domain.tps.record.Record9910;
 ...
 * Created by zhanrui
 * 20150316.
 * 1381000查询还款明细 测试交易  Linking直接返回
 */
public class TX001Processor extends AbstractTxnProcessor {
    ...
    protected void doRequest(Stdp10ProcessorRequest request, Stdp10ProcessorResponse response) throws ProcessorException, IOException {
        ...
        try {
            ...
            //---------
            cbsRtnInfo = new CbsRtnInfo();
            List<CbsToa1000Item> itemList = new ArrayList<>();
            CbsToa1000Item item = new CbsToa1000Item();
            item.setAcctNo("acctno001");
            item.setCustName("客户姓名1");
            item.setTxnAmt(new BigDecimal("0.01"));
            itemList.add(item);
            item = new CbsToa1000Item();
            item.setAcctNo("acctno002");
            item.setCustName("客户姓名2");
            item.setTxnAmt(new BigDecimal("0.02"));
            itemList.add(item);

            toa.setItemNum("2");
            toa.setItems(itemList);
            toa.setTotalAmt(new BigDecimal("0.03"));
